    ...
    <div className={`markdown-content space-y-2 leading-relaxed text-xs md:text-sm text-[#27272A] ${className}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          em: ({ children }) => <em className="italic text-[#52525B]">{children}</em>,
          blockquote: ({ children }) => (
          ...
          table: ({ children }) => (
            <div className="overflow-x-auto my-3 border-2 border-[#27272A] rounded-lg shadow-[2px_2px_0px_#27272A]">
              <table className="min-w-full divide-y-2 divide-[#27272A] bg-white text-xs">{children}</table>
            </div>
          ),
          thead: ({ children }) => <thead className="bg-amber-100 font-extrabold text-[#27272A]">{children}</thead>,
          tbody: ({ children }) => <tbody className="divide-y divide-[#27272A]/20">{children}</tbody>,
          tr: ({ children }) => <tr className="hover:bg-amber-50/50 transition-colors">{children}</tr>,
          th: ({ children }) => <th className="px-3 py-2 text-left font-extrabold text-[#27272A]">{children}</th>,
          td: ({ children }) => <td className="px-3 py-2 text-[#27272A] font-medium">{children}</td>,
        }}
      >
        {cleanedText}
      </ReactMarkdown>
