...
export function AppSidebar() {
  ...
  const handleSaveRename = async (convId: string, e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${OpenAPI.BASE}/api/v1/agent/conversations/${convId}`, {
        method: "PATCH",
        headers: {
  ...
  return (
    <Sidebar collapsible="none" className="border-r-2 border-[#27272A] bg-[#F4EFE6] text-[#27272A] h-screen h-full flex flex-col justify-between">
      ...
      <SidebarContent className="flex flex-col justify-between flex-1 bg-[#F4EFE6]">
              ...
              {conversations.map((conv) => (
                <SidebarMenuItem key={conv.id}>
                  {editingConvId === conv.id ? (
                    <form onSubmit={(e) => handleSaveRename(conv.id, e)} className="flex items-center gap-1.5 w-full py-1">
                      <Input
                      ...
                      />
                      <Button type="submit" size="icon" className="h-7 w-7 bg-emerald-600 hover:bg-emerald-700 text-white shrink-0">
                    ...
                    <SidebarMenuButton
                      asChild
                      isActive={location.pathname === "/chat" && activeConvId === conv.id}
                    ...
                    >
                      <Link to="/chat" search={{ convId: conv.id } as any}>
                        <div className="flex items-center gap-2 truncate">
                          ...
                          <span className="truncate font-semibold text-[#27272A]">{conv.title}</span>
                        </div>
        ...
        <SidebarGroup className="p-2 space-y-1 mt-auto bg-[#F4EFE6]">
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">
              <SidebarMenuItem>
                <SidebarMenuButton asChild isActive={location.pathname === "/portfolios"}>
                  <Link to="/portfolios" className="flex items-center gap-2.5 text-xs font-bold text-[#27272A] p-2 hover:bg-amber-100 rounded transition-colors">
              ...
              <SidebarMenuItem>
                <SidebarMenuButton asChild isActive={location.pathname === "/watchlists"}>
                  <Link to="/watchlists" className="flex items-center gap-2.5 text-xs font-bold text-[#27272A] p-2 hover:bg-amber-100 rounded transition-colors">
              ...
              {currentUser?.is_superuser && (
                <SidebarMenuItem>
                  <SidebarMenuButton asChild isActive={location.pathname === "/admin"}>
                    <Link to="/admin" className="flex items-center gap-2.5 text-xs font-bold text-[#27272A] p-2 hover:bg-amber-100 rounded transition-colors">
